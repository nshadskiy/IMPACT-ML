...
def get_mass_combinations(masses_X: List[int], masses_Y: List[int]) -> List[Dict]:
    ...
    Args:
        masses_X: List on masses for X resonance
        masses_Y: List on masses for Y resonance

    Return:
        List of mass combinations for X and Y
    """
    mass_combinations = list()
...
def add_mass_variables(rdf: Any, sample: str) -> Any:
    """
    This function adds a column with NMSSM signal mass points information. Negative value is used for all other samples.

    Args:
        rdf: root DataFrame object
        sample: Exact name of the folder where the ntuples are stored e.g. "TauEmbedding-MuTauFinalState_Run2018A-UL2018"

    Return:
        root DataFrame object with new mass variables
    """
    if "NMSSM" in sample:
        mX = sample.rsplit("_")[2].replace("MX-", "")
        mY = sample.rsplit("_")[3].replace("MY-", "")

        rdf = rdf.Define("massX", mX)
        rdf = rdf.Define("massY", mY)
    else:
        rdf = rdf.Define("massX", "-10")
        rdf = rdf.Define("massY", "-10")

    return rdf


def get_mass_grid(sample_list: List[str]) -> Dict[str, set]:
    """
    This function generates a dictionary of mass points based on a list of sample names.

    Args:
        sample_list: List of sample names with a concrete structure e.g. "NMSSM_XToYHTo2Tau2B_MX-450_MY-250_*"

    Return:
        Dictionary of processed mass points
    """
    grid = {"mX": set(), "mY": set()}
